...
for n in (a):
	...
	cursor.execute('SELECT mii.entryno, mii.initial, mii.permlikes, mii.skill, mii.country, mii.miidata, artisan.miidata, artisan.craftsno, artisan.master FROM mii, artisan WHERE mii.craftsno = artisan.craftsno AND mii.permlikes<25 ORDER BY RAND() LIMIT %s', [n]) #idk how this works it needs to be figured out
...
for t in (a):
	...
	cursor.execute('SELECT mii.entryno, mii.initial, mii.permlikes, mii.skill, mii.country, mii.miidata, artisan.miidata, artisan.craftsno, artisan.master FROM mii, artisan WHERE mii.craftsno = artisan.craftsno ORDER BY entryno DESC LIMIT %s', [t])
	miilist = cursor.fetchall()
