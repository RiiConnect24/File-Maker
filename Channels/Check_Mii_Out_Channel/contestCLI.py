...
def searchContest():
    search = str(
        input(
            "-> Please enter a keyword or phrase in the contest's topic or description: "
        )
    )
    formattedSearch = "%" + search.replace(" ", "%") + "%"
    cursor.execute(
        "SELECT id, description FROM contests WHERE topic LIKE %s OR description LIKE %s",
...
def showInfo(id):
    cursor.execute(
        "SELECT start, end, status, entrycount, topic, description FROM contests WHERE id = %s",
        [id],
    )
...
if entry == 1:
    ...
    while True:
        month = input(
            "-> Enter starting month (leave blank to start when next contest changes status): "
        )

        if len(month) == 0:
            currentTime = int(mktime(datetime.utcnow().timetuple())) - 946684800
            cursor.execute(
                "SELECT end, status, description FROM contests WHERE end > %s AND status != 'closed' ORDER BY end ASC LIMIT 1",
                [currentTime],
            )
            ...
            if len(nextContest) == 0:
                print(
                    "There are no future contests scheduled. Please try again and enter the date manually."
                )
                exit()
            ...
            startDate = datetime.fromtimestamp(start + 946684800).date()
                ...
                start + (7 * 24 * 60 * 60)
            )  # change status every 7 days until closure
            endDate = datetime.fromtimestamp(end + 946684800).date()

            if nextContest[1] == "waiting":
                nextStatus = "open"
            elif nextContest[1] == "open":
                nextStatus = "judging"
            elif nextContest[1] == "judging":
                nextStatus = "results"
            elif nextContest[1] == "results":
                nextStatus = "closed"

            print(
                "\nThe contest '{}' will become status '{}' on {}.".format(
                    nextContest[2], nextStatus, startDate
                )
            )
            choice = str(
                input(
                    "-> This new contest will begin at this date. Is this correct? [Y/N]: "
                )
            )
            if choice.upper() != "Y":
            ...
            try:
                ...
                print(
                    f"{colors.FAIL}Invalid month!{colors.END} Make sure you are entering the month by its number."
                )
                continue
            ...
            if (
                (month != 1 and datetime.now().month == 12)
    ...
    while True:
        ...
        if len(topic) > 10:
            print(
                "The contest topic cannot be longer than 10 characters. That topic is",
                len(topic),
                "characters.",
            )
    ...
    while True:
        ...
        if len(description) > 64:
            print(
                "The contest description cannot be longer than 64 characters. That topic is",
                len(description),
                "characters.",
            )
    ...
    if choice.upper() == "Y":
        cursor.execute(
            "INSERT INTO contests(start, end, status, entrycount, topic, description) VALUES (%s, %s, %s, %s, %s, %s)",
            (start, end, "waiting", 0, topic, description),
        )
        db.commit()
    ...
    while not stop:
        ...
        if selection == 1:
            ...
            if editSelection == 1:
                ...
                cursor.execute(
                    "UPDATE contests SET topic = %s WHERE id = %s", (topic, id)
                )
                db.commit()
                ...
                while True:
                    description = str(
                        input("-> Enter a new description for the contest: ")
                    )
                    if len(description) > 64:
                        print(
                            "The contest topic cannot be longer than 64 characters. That topic is",
                            len(description),
                            "characters.",
                        )
                    else:
                ...
                cursor.execute(
                    "UPDATE contests SET description = %s WHERE id = %s",
                    (description, id),
                )
                db.commit()
                print(
                    f"{colors.GREEN}Contest description updated successfully.{colors.END}"
                )
                stop = True
    ...
    while not stop:
        ...
        if selection == 1:
            id = int(input("-> Enter contest ID: "))
            cursor.execute(
                "SELECT status, entrycount FROM contests WHERE id = %s", [id]
            )
            result = cursor.fetchone()
            if result == None:
            ...
            if result[0] != "closed":
                print(
                    "WARNING: This contest is still active with status '{}'.".format(
                        result[0]
                    )
                )
            ...
            if int(result[1]) != 0:
                print(
                    "This contest and its",
                    result[1],
                    "miis will be permanently deleted.",
                )
            ...
            choice = str(
                input(
                    f"-> Are you sure you want to {colors.UNDERLINE}delete{colors.END} contest #{id}? [Y/N]: "
                )
            )
