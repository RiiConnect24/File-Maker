...
for i in range(len(count)):  # add the artisan data to each mii based on their craftsno
    cursor.execute(
        "SELECT master,lastpost FROM artisan WHERE craftsno = %s", [count[i][0]]
    )
    ...
    lastpost = int(
        (
            ...
            - result[1].replace(tzinfo=timezone.utc).timestamp()
        )
        / 60
        / 60
    )  # hours since artisan last posted
    master = int(result[0])
    ...
    cursor.execute(
        "SELECT craftsno,miidata,popularity,country FROM artisan WHERE craftsno = %s",
        [count[i][0]],
    )
    artisanData = cursor.fetchone()
    artisanData = (
        artisanData[:2] + (master,) + artisanData[2:]
    )  # insert master into the tuple where it would normally be retrieved from the db lmao
