...
now = datetime.utcnow()
boundary = now.strftime("--BoundaryForDL%Y%m%d%H%m/" + str(randint(1000000, 9999999)))
...
def enc(text, description):
    return str(
        encodebytes(text.format(description).encode("utf-16be"))
        .replace(b"\n", b"\r\n")
        .decode("utf-8")
    )
...
for c in contests:
    ...
    if sent == 0:
        ...
        data = {
            "username": "CMOC Bot",
            "content": contest_status,
            "avatar_url": "http://rc24.xyz/images/logo-small.png",
            "attachments": [
                {
                    "fallback": contest_status,
                    "color": "#CB7931",
                    "author_name": "RiiConnect24 CMOC Script",
                    ...
                    "fields": [
                        {
                            "title": "Script",
                            "value": "Check Mii Out Channel",
                            "short": "false",
                        }
                    ],
                    "thumb_url": "https://miicontest.wii.rc24.xyz/photo/{}.jpg".format(
                        contest_id
                    ),
