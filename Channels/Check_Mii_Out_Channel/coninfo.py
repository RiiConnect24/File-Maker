...
def setRankings(id):  # once voting is concluded, rank miis from 1 to 10
    cursor.execute('UPDATE conmiis SET likes = (SELECT COUNT(*) FROM convotes WHERE id = %s AND (craftsno1 = conmiis.craftsno OR craftsno2 = conmiis.craftsno OR craftsno3 = conmiis.craftsno)) WHERE contest = %s', (id, id))  # adds all the mii's votes
    db.commit()
    cursor.execute(
    ...
    for i in range(miiCount):
        ...
        cursor.execute(
            'UPDATE conmiis SET `rank` = %s WHERE entryno = %s', (percentile, miis[i][0]))
...
for con in cursor.fetchall():
    ...
    if status == 'waiting' and start <= currentTime and end >= currentTime:  # contest is ready to be opened
        cursor.execute(
            'UPDATE contests SET status = \'open\' WHERE id = %s', [id])
        ...
        if status == 'open':
            cursor.execute(
                'SELECT COUNT(*) FROM conmiis WHERE contest = %s', [id])
            cursor.execute('UPDATE contests SET status = \'judging\', end = %s, entrycount = %s, sent = 0 WHERE id = %s',
                           (endTime, cursor.fetchone()[0], id))

        elif status == 'judging':
            cursor.execute(
                'UPDATE contests SET status = \'results\', end = %s, sent = 0 WHERE id = %s', (endTime, id))

        elif status == 'results':
            cursor.execute(
                'UPDATE contests SET status = \'closed\' WHERE id = %s', [id])
