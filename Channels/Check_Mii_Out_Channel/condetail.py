...
cursor.execute('SELECT id, start, end, status, entrycount, topic, description FROM contests WHERE status != \'closed\' AND status != \'waiting\'')
...
for i in contests:
	...
	with open('{}/contest/{}/con_detail1.ces'.format(config['miicontest_path'], id), 'wb') as file:
