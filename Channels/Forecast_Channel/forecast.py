...
def get_legacy_api(list, key):
    ...
    index = [[2, 3], [2, 2], [3, 3], [3, 2], [4, 3], [4, 2], [5, 3], [5, 2]]
    for day in range(0, 8):
        week[key][day] = int(forecast[index[day][0]][5][index[day][1]].text)
    for i in range(0, 8):
        week[key][i + 10] = to_celsius(week[key][i])
    for i in range(20, 24):
        week[key][i] = get_icon(int(forecast[i - 18][5][1].text), list, key)
    ...
    for i in range(25, 29):
        week[key][i] = int(forecast[6 if i <= 26 else 7][5][2 if i % 2 == 1 else 3].text)
    for i in range(29, 33):
        week[key][i] = int(to_celsius(week[key][i - 4]))
...
def get_tenki_data(key, lat, lon):
    ...
    response = json.loads(requests.get("https://static.tenki.jp/static-api/app/forecast-{}.json".format(jiscode)).content)["indexes"]["cloth_dried"]["url"]

    return None # I'm rewriting this part of the code.
