...
"""This is to check if a character is in the Wii font. If it isn't, it'll be replaced with a unidecode."""
...
def download_reuters_america_english():
    ...
    topics["world"] = ["worldNews"]
    topics["us"] = ["domesticNews"]
    topics["health"] = ["healthNews"]
    topics["science"] = ["scienceNews"]
...
def download_reuters_europe_english():
    print "Downloading from Reuters (Europe English)...\n"
...
def download_reuters_japanese():
    print "Downloading from Reuters (Japanese)...\n"

    topics_name = collections.OrderedDict()

    topics_name["world"] = "ワールド"
    topics_name["business"] = "ビジネス"
    topics_name["sports"] = "スポーツ"
    topics_name["technology"] = "テクノロジー"
    topics_name["entertainment"] = "エンタテインメント"

    topics = collections.OrderedDict()

    topics["world"] = ["ReutersJpWorld"]
    topics["business"] = ["ReutersJpBiz"]
    topics["sports"] = ["ReutersJpSports"]
    topics["technology"] = ["ReutersJpTech"]
    topics["entertainment"] = ["ReutersJpEnt"]

    return download_reuters("jp", topics_name, topics)

def download_reuters(language, topics_name, topics):
    ...
    for rss_category in topics.items():
        ...
        for rss in rss_category[1]:
            ...
            if language == "en": url = "http://feeds.reuters.com/reuters/%s.rss" % rss
            elif language == "jp": url = "https://twitrss.me/twitter_user_to_rss/?user=%s" % rss
            ...
            for items in rss_feed.entries:
                ...
                if updated >= time_current - 60:
                    ...
                    if language == "jp": items["link"] = "http://bit.ly/" + items["description"].split("http://bit.ly/", 1)[1][:7]

                    parsedata = parsedata_reuters(language, items["link"], items["title"], updated)

                    if parsedata is not None: data[rss_category[0] + str(numbers)] = parsedata

        print "\n"

    return data

def parsedata_reuters(language, url, title, updated):
    utc = pytz.utc
    ...
    try:
        c = soup.find("span", {"class": "Image_caption_KoNH1"})
        caption = c.contents[0]
        c.decompose()
    except:
        pass

    try:
        soup.find("div", {"class": "StandardArticleBody_trustBadgeContainer_1gqgJ"}).decompose()
    except:
        pass

    try:
        soup.find("div", {"class": "Slideshow_inline-container_1QqKC Slideshow_mega_19SOz"}).decompose()
    except:
        pass
    ...
    article_text = BeautifulSoup(str(soup.find("div", {"class": "StandardArticleBody_body_1gnLA"})).replace("</p>", "\n\n</p>"), "lxml").get_text()
    ...
    try:
        ...
        elif language == "jp": location = article.decode("utf-16be").split("[")[1].split("　")[0]
    ...
    try:
        ...
        if language == "en": caption = fix_chars(caption)
    ...
    if len(headline) == 0:
        capture_message("Headline is blank. %s" % url, "warning")
        return None
    elif len(article) == 0:
        capture_message("Article is blank. %s" % url, "warning")
        return None
...
def download_efe(mode, topics_name):
    ...
    for items in rss_feed.entries:
        ...
        if updated >= time_current - 60:
            ...
            if parsedata is not None: data[category + str(numbers)] = parsedata
...
def download_zeit(topics_name):
    ...
    for items in soup.findAll("article", {"class": "newsteaser"}):
        ...
        if updated >= time_current - 60:
            ...
            if parsedata is not None: data[parsedata[9] + str(numbers)] = parsedata
