...
def make_timestamps_table(mode, topics_table, topics_news):
    ...
    def timestamps_table_add(topics):
        ...
        return timestamps, len(times_list)
    ...
    for topics in list(topics_news.values()):
        ...
        if timestamps_add[1] != 0: # might be timestamps_add[0] but why are you comparing a returned string and a number? 0 != "" in python
            topics_table["topics_%s_article_number" % numbers] = u32(timestamps_add[1])
            topics_table[
                "topics_%s_article_offset" % numbers] = offset_count()  # Offset for the articles to choose for the topic.
            timestamps_table["timestamps_%s" % numbers] = timestamps_add[0]  # Timestamps.
...
def make_topics(topics_table, topics_news):
    ...
    for keys in list(topics_news.keys()):
        ...
        topics["topics_%s_read" % numbers] = keys.encode("utf-16be")  # Read the topics.
...
def make_source_name_copyright(source_table, data):
    ...
    copyrights = {
        "AP": (
                "Copyright %s The Associated Press. All rights reserved. This material may not be published, broadcast, rewritten or redistributed." % date.today().year)
            .encode("utf-16be"),
        "Reuters": (
                "© %s Thomson Reuters. All rights reserved. Republication or redistribution of Thomson Reuters content, including by framing or similar means, is prohibited without the prior written consent of Thomson Reuters. Thomson Reuters and the Kinesis logo are trademarks of Thomson Reuters and its affiliated companies." % date.today().year)
            .encode("utf-16be"),
        "AFP": (
                "All reproduction and representation rights reserved. © %s Agence France-Presse" % date.today().year)
            .encode("utf-16be"),
        "AFP_French": (
                "Tous droits de reproduction et de diffusion réservés. © %s Agence France-Presse" % date.today().year)
             .encode("utf-16be"),
        "ANP": (
                "All reproduction and representation rights reserved. © %s B.V. Algemeen Nederlands Persbureau ANP" % date.today().year)
            .encode("utf-16be"),
        "ANSA": (
                "© %s ANSA, Tutti i diritti riservati. Testi, foto, grafica non potranno essere pubblicali, riscritti, commercializzati, distribuiti, videotrasmessi, da parte dagli tanti e del terzi in genere, in alcun modo e sotto qualsiasi forma." % date.today().year)
            .encode("utf-16be"),
        "SID": (
                "Alle Rechte für die Wiedergabe, Verwertung und Darstellung reserviert. © %s SID" % date.today().year)
            .encode("utf-16be"),
        "dpa": (
                "Alle Rechte für die Wiedergabe, Verwertung und Darstellung reserviert. © %s dpa" % date.today().year)
            .encode("utf-16be"),
        "NU.nl": (
                "© %s Sanoma Digital The Netherlands B.V. NU - onderdeel van Sanoma Media Netherlands Group" % date.today().year)
            .encode("utf-16be"),
        "Reuters_Japanese": (
                "© Copyright Reuters %s. All rights reserved.　ユーザーは、自己の個人的使用及び非商用目的に限り、このサイトにおけるコンテンツの抜粋をダウンロードまたは印刷することができます。ロイターが事前に書面により承認した場合を除き、ロイター・コンテンツを再発行や再配布すること（フレーミングまたは類似の方法による場合を含む）は、明示的に禁止されています。Reutersおよび地球をデザインしたマークは、登録商標であり、全世界のロイター・グループの商標となっています。 " % date.today().year)
            .encode("utf-16be"),
    }
        ...
        if article[8] not in source_articles:
            if article[8] in sources:
                ...
                with open("./Channels/News_Channel/logos/%s.jpg" % article[8], "rb") as source_file:
                    ...
                    source_pictures["logo_%s" % article[8]] = image
    ...
    for dictionary in dictionaries:
        for values in list(dictionary.values()):
            with open(newsfilename + "-1", "ba+") as dest_file:
                dest_file.write(str(values).encode("utf-8")) # I challenge you to get this working
