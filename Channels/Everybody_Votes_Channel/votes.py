    ...
    for i in results:
        if results[i][8] == "n":
            for j in range(region_number[country_code]):
                ...
                table["position_entry_table_count_%s" % num()] = u8(0 if (results[i][6][country_index][j] == 0 and results[i][7][country_index][j] == 0) or (country_code not in position_table.keys()) else 22 if country_code == 77 else position_table[country_code][j])
